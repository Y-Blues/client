...
## Découverte des composants du backend, création des proxys au vol

Historiquement, `components.py` créait les quatre `Remote*` **inconditionnellement** — sans jamais vérifier que le backend appelé les a réellement chargés. Depuis la v4 (spec [§10](docs/superpowers/specs/2026-09-15-client-design.md#10-v4-2026-09-16--découverte-des-composants-du-backend-création-des-proxys-au-vol)), `ycappuccino.client.discovery` peut interroger le backend **avant** que le framework ne démarre, et ne créer que les proxys correspondant aux interfaces réellement présentes.

**Comment ça marche** : `static/main.py` appelle `discovery.prepare_generated_module(...)` juste après l'installation des paquets, **avant** `Framework().init(...)`. Cette fonction tente `IServiceEndpoint.call("__remote_capabilities__", "POST", [], {}, {}, subject=None)` — construit à la main, sans Pelix, exactement comme les tests unitaires par composant de ce dépôt — via la route déjà générique `/api/services/<nom>` (aucun code spécial requis pour appeler un service dont le nom n'est pas connu à l'avance). Le résultat décide quels `make_remote(...)` écrire dans un module Python **ordinaire** (`bundle/generated_remote.py`), que le scan `bundle_prefix` du framework découvre ensuite normalement, exactement comme `components.py` aujourd'hui — rien de dynamique ne se produit pendant ou après ce scan, donc le piège de timing documenté par `core/README.md` (« Piège de timing », créer un composant depuis le `start()` d'un autre) ne s'applique tout simplement pas à ce mécanisme.

**Si l'appel échoue, pour n'importe quelle raison** (le backend ne charge pas `ycappuccino.remote`, n'a pas câblé ce nom précis, erreur réseau, réponse mal formée) : repli intégral, automatique, sur le comportement historique — les quatre `Remote*` inconditionnels, exactement comme avant cette version. `components.py` lui-même n'a pas changé de comportement observable (juste refactorisé pour tirer sa table `interface → resource` de `known_interfaces.KNOWN_INTERFACES`, désormais partagée avec `discovery.py`).

**Limites honnêtes, à ne pas édulcorer** :

- **`"__remote_capabilities__"` est une convention, pas un contrat.** `client` n'importe pas `ycappuccino.remote` et ne connaît pas sa forme exacte ; ce nom (et la forme supposée de sa réponse, un flux de `{"module", "class", "provides"}` identique à `Framework.list_components()`) est une hypothèse non vérifiée contre une implémentation réelle de `remote` (dépôt en cours d'extension, en parallèle, hors du périmètre de cet agent). Si cette convention change, la découverte se dégrade silencieusement vers le repli — c'est voulu, pas un bug.
- **La création au vol ne couvre que les quatre interfaces déjà connues** (`ICrud`/`IDrafts`/`IItemCatalog`/`IServiceEndpoint`, table `known_interfaces.KNOWN_INTERFACES`, triviale à étendre le jour où une vraie cinquième interface `api` obtient un montage HTTP réel). `client` peut désormais **sauter** une de ces quatre (si le backend ne l'a pas), mais ne peut **pas** proxyfier une interface applicative arbitraire découverte via `__remote_capabilities__` : `make_remote` a structurellement besoin d'un préfixe de montage HTTP (`resource`, spec §9.1) qu'aucune réflexion sur les noms de méthodes ne peut deviner, et qui n'existe, aujourd'hui, que pour ces quatre routes REST d'`http_server`. Une interface sur mesure n'a **aucune forme HTTP connue** de `client` — contrairement au cas `remote` (pairs serveur-à-serveur, tous deux capables de faire tourner un dispatcher RPC générique), un navigateur appelant l'API REST d'`http_server` n'a pas cette option. Le vrai suivi légitime — `http_server` grandissant une route RPC générique — est hors du périmètre de cet agent (`http_server` non modifié ici) et reste à faire.
